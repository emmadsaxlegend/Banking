...
Router.post('/profile', authMiddleware, async (req,res) => {
    try {
        ...
        const validFieldsToUpdate = ['name', 'phone']
        ...
        if (isInvalidFieldProvided || !name || !phone ) {
            return res.status(400).send({
              update_error: 'Invalid field.'
            });
        }
        const result = await BankUser.findByIdAndUpdate(req.user._id, { name, phone }, { new : true }).select('-password')
